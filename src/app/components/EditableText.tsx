...
export function exportFullState(
  ...
  // Step 1: Collect ALL unique keys from all sources
  const allKeysSet = new Set<string>();
  Object.keys(defaults).forEach(k => allKeysSet.add(k));
  Object.keys(overrides).forEach(k => allKeysSet.add(k));
  Object.keys(registry).forEach(k => allKeysSet.add(k));
  Object.keys(edits).forEach(k => allKeysSet.add(k));
  
  // Step 2: For each key, determine final value using lookup priority
  allKeysSet.forEach(key => {
    // Priority: edits → overrides → registry → defaults
    if (edits[key]) {
      state[key] = edits[key].to;
    } else if (overrides[key] !== undefined) {
      state[key] = overrides[key];
    } else if (registry[key] !== undefined) {
      state[key] = registry[key];
